...
#include <boost/test/included/unit_test.hpp>  // include this to get 
#include "lab1.h"
...
vector <string> FuncForTest(string s, string subs)
{
	if (SearchElement(s, subs))
	{
		...
		else res.insert(res.begin(), "Any string \n"); // subs == "*"
	}
	else res.insert(res.begin(), "No match \n");
	return res;
}
...
{
	vector <string> rr;
	rr.insert(rr.end(),"Pos: 5 aaaacbbbbaccc");
	rr.insert(rr.end(),"Pos: 5 aaaacbbbbacccc");
	rr.insert(rr.end(),"Pos: 6 aaacbbbbaccc");
	rr.insert(rr.end(),"Pos: 6 aaacbbbbacccc");	
    BOOST_CHECK(FuncForTest("ccccdaaaacbbbbaccccaaa", "aaa*bbb*ccc") == rr);
}
